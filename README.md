...
- **左: 文字起こし** — 「録音開始」で話すと、区切りごとに行が増えます。
  ...
  語彙ヒント（whisper への認識ヒント）は、**対象業務名・登場人物名・書類やシステムの名前を、図が育つのに
  合わせて自動で足します**（`lib/transcript/vocab.ts`。Jev/gemma をすでに経た語彙なので、新たに呼び直さない）。
  「追加の語彙ヒント」欄は、まだ図に出ていない語彙だけを手で足す場所で、自動の分を上書きしません。
