...
トップページ（`/`）で「業務名」と「関係部署」を入れて会議を始めます（「サンプルで始める」もあります）。
保存済みの会議は一覧から開いて再開でき、削除は確認つきです。会議は `/s/<slug>` で開きます。
...
- **題材「このアプリの仕組み」（Jev 専用）**: このアプリ自身の仕組みを業務に見立てた台本です。
  ...
  トップページの「アプリの仕組みで始める」で、この 6 者を関係部署にした会議を作れます（他の初期設定の会議で題材だけ切り替えると、6 者が新しい登場人物として扱われ、ほぼ全部が仮ステップになります）。
...
確信の低いステップは点線の「（仮）」で描かれ、図の下の「確認待ちのステップ」に **承認 / 却下** が出ます。
承認すると確定（実線）、却下すると取り消し（履歴には残ります）。
例外・暗黙知・属人化の印が付いたステップは、名前の末尾に［例外］［暗黙知］［属人］が付きます。

### 業務分掌ドキュメント

図ペインの「業務分掌を作る」で、Markdown を作ります（表示・`.md` 保存・コピー）。
