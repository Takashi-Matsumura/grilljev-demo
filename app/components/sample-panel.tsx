...
const MODE_HINT: Record<SampleMode, string> = {
  script: "台本に固定で書いた変更を図に反映します（Jev は呼びません。課金なし）。",
  jev: "1 行ごとに Jev を呼んで判定します（外部 API・課金あり）。名前はローカルの gemma が作り、次の発話で Jev が検証します。",
};

/** 開発・デモ用。マイクなしで、台本の文字起こしを流して図を育てる。 */
    ...
    <details open className="border-t border-black/10 px-4 py-3 dark:border-white/15">
      ...
      <div className="mt-3 flex flex-wrap items-center gap-3">
        ...
        <label
          ...
          title="対象業務の変化の検知を試す用。Jev モードでだけ検知します（台本モードでは図は変わりません）。"
        >
          <input type="checkbox" checked={withShift} onChange={(e) => onWithShiftChange(e.target.checked)} />
          話題転換を足す
        </label>
      </div>
